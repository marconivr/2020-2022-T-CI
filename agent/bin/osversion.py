...
def main():
    ...
    for arg in sys.argv:
        ...
        if "--help" in arg:
            ...
            print("--outputfile=[output file] | Selects the output file. Default to 'osversion.csv'")
            print("--outputfolder=[output folder] | Selects the output folder. Default to 'osversion.csv'")
            print("\nAdditional flags:")
    ...
    log(f"osversion.py inizialized, verbose {verbose}")
    start_time = time()
    ...
    with open(file_out, "a+") as fm:
        ...
        for i in range(len(machines)):
            ...
            if is_reachable(machines[i]):
                try:
                    ...
                    for osversion in con.Win32_OperatingSystem():
                        csv_row = ""

                        # filter (and prints) only user's parameters
                        if vonly is not None:
                            string = f"{machines[i]:<15} {str(osversion.Name):<65}"
                            for attribute in vonly.split(","):
                                for instance in (str(osversion).split(";")):
                        ...
                        csv_row += f'"{osversion.Caption}";'
                        csv_row += f'"{osversion.Version}";'
                        csv_row += f'"{osversion.SerialNumber}";'
                        csv_row += f'"{osversion.SystemDrive}";'
                        csv_row += f'"{osversion.Organization}";'
                        csv_row += f'"{osversion.OSArchitecture}"\n'
        ...
        fm.close()
        end_time = time()
        print_log(f"osversion.py done in {round(end_time - start_time, 4)} s")
    log()
    exit(0)
