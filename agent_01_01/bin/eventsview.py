...
def main():
    ...
    for arg in sys.argv:
        ...
        if "--help" in arg:
            ...
            print("--outputfile=[output file] | Selects the output file. Default to 'eventsview.csv'")
            print("--outputfolder=[output folder] | Selects the output folder. Default to 'eventsview.csv'")
            print("--logfile=[logfile] | Select the log file")
    ...
    log(f"eventsview.py inizialized, verbose {verbose}")
    start_time = time()
    ...
    with open(file_out, "a+") as fm:
        ...
        for i in range(len(machines)):
            ...
            if is_reachable(machines[i]):
                try:
                    ...
                    for eventsview in con.Win32_NTLogEvent(Logfile=lfile):
                        csv_row = ""

                        # filter (and prints) only user's parameters
                        if vonly is not None:
                            string = f"{machines[i]:<15} {str(eventsview.Name):<65}"
                            for attribute in vonly.split(","):
                                for instance in (str(eventsview).split(";")):
                        ...
                        csv_row += f'"{eventsview.Category}";'
                        csv_row += f'"{eventsview.CategoryString}";'
                        csv_row += f'"{eventsview.ComputerName}";'
                        csv_row += f'"{eventsview.EventType}";'
                        csv_row += f'"{eventsview.SourceName}";'
                        csv_row += f'"{eventsview.Type}";'
                        csv_row += f'"{eventsview.User}";'
                        csv_row += f'"{eventsview.RecordNumber}"\n'
        ...
        fm.close()
        end_time = time()
        print_log(f"eventsview.py done in {round(end_time - start_time, 4)} s")
    log()
    exit(0)
