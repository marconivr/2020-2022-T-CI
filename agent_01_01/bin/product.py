...
def main():
    ...
    for arg in sys.argv:
        ...
        if "--help" in arg:
            ...
            print("--outputfile=[output file] | Selects the output file. Default to 'product.csv'")
            print("--outputfolder=[output folder] | Selects the output folder. Default to 'product.csv'")
            print("\nAdditional flags:")
    ...
    log(f"product.py inizialized, verbose {verbose}")
    start_time = time()
    ...
    with open(file_out, "a+") as fm:
        ...
        for i in range(len(machines)):
            ...
            if is_reachable(machines[i]):
                try:
                    ...
                    for product in con.Win32_Product():
                        csv_row = ""

                        # filter (and prints) only user's parameters
                        if vonly is not None:
                            string = f"{machines[i]:<15} {str(product.Name):<65}"
                            for attribute in vonly.split(","):
                                for instance in (str(product).split(";")):
                        ...
                        csv_row += f'"{product.Caption}";'
                        csv_row += f'"{product.HelpLink}";'
                        csv_row += f'"{product.IdentifyingNumber}";'
                        csv_row += f'"{product.InstallDate}";'
                        csv_row += f'"{product.InstallLocation}";'
                        csv_row += f'"{product.InstallSource}";'
                        csv_row += f'"{product.InstallState}";'
                        csv_row += f'"{product.Language}";'
                        csv_row += f'"{product.LocalPackage}";'
                        csv_row += f'"{product.PackageCode}";'
                        csv_row += f'"{product.PackageName}";'
                        csv_row += f'"{product.ProductID}";'
                        csv_row += f'"{product.RegOwner}";'
                        csv_row += f'"{product.RegCompany}";'
                        csv_row += f'"{product.Version}";'
                        csv_row += f'"{product.Vendor}"\n'
        ...
        fm.close()
        end_time = time()
        print_log(f"product.py done in {round(end_time - start_time, 4)} s")
    log()
    exit(0)
